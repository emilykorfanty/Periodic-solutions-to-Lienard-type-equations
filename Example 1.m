...
% Calculate average value of f
ave_f = mean(f);

maxk = sqrt(12)/(T^2) - M; % for reference
...
for i=1:floor(N/4)
  line(x(4*i)*ones(1, 100), linspace(K1(4*i), plotH), 'Color', 'c');
end

for i=1:floor(N/4)
  line(x(4*i)*ones(1, 100), linspace(-plotH, K2(4*i)), 'Color', 'c');
end
...
line(R*ones(1, 100), linspace(AVE(3*N/4), plotH, 100), 'Color', 'y');

for i = 1 : floor(N/32)
   line(x(3*N/4  + 8*i)*ones(1, 100), linspace(AVE(3*N/4  + 8*i), plotH, 100), 'Color', 'y');
end

line(-R*ones(1, 100), linspace(AVE(N/4), plotH, 100), 'Color', 'y');
...
% create the legend
legend(hleglines,'b = 0.115, c = -0.05')%,'b = 2, c = 1.75','b = 2.75, c = 3');
